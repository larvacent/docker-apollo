...
RUN set -x \
	...
	&& tar zxf apache-apollo-${APOLLO_VERSION}-unix-distro.tar.gz -C /usr/local && rm -rf apache-apollo-${APOLLO_VERSION}-unix-distro.tar.gz \
	&& mv /usr/local/apache-apollo-${APOLLO_VERSION} /usr/local/apollo